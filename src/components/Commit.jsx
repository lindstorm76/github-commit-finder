import React from "react"

const Commit = ({ commitSha, dateTime, message, latest }) => {
  
  const wrapWord = (word, len) => (
    word.length <= len ? word : word.substring(0, len) + "..."
  )

  return (
    <div className="card m-md-4 m-2" >
      <h5 className="card-header position-relative pt-4">
        <div style={{ top: -10, left: 10 }} className="position-absolute" >
          <span style={{ backgroundColor: "#10B981" }} className="badge">{dateTime}</span>
          {/* <span style={{ backgroundColor: "#F59E0B" }} className="badge ms-2">repo</span> */}
          {latest && <span style={{ backgroundColor: "#EF4444" }} className="badge ms-2">Latest</span>}
        </div>
        {commitSha}
      </h5>
      <div className="card-body">
        <p className="card-text">{message}</p>
        <a target="_blank" href={`https://github.com/lindstorm75/dota2heroes/commit/${commitSha}`} className="btn btn-primary">Commit page</a>
      </div>
    </div>
  )
}

export default Commit